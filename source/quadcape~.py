...
def idle_tuning():
	...
	#Code to filter data; makes first numPoints data points the first measurement
	numPoints = 10
...
def run():
	...
	new_data_gx,new_data_gy,new_data_gz,t=Gyro.get() #degrees/sec
	new_data_ax,new_data_ay,new_data_az=Accel.get() #in G 	
	new_data_gx,new_data_gy,new_data_gz=new_data_gx+gx_os,new_data_gy+gy_os,new_data_gz+gz_os
	new_data_ax,new_data_ay,new_data_az=new_data_ax+ax_os,new_data_ay+ay_os,new_data_az+az_os
	
	gx_up=gx.update(data_gx.filter(new_data_gx))
	gy_up=gy.update(data_gy.filter(new_data_gy))
	gz_up=gz.update(data_gz.filter(new_data_gz))
	ax_up=ax.update(data_ax.filter(new_data_ax))
	ay_up=ay.update(data_ay.filter(new_data_ay))
	az_up=az.update(data_az.filter(new_data_az))
