...
def init_menus():	
	...
	#menu_functions=[2tuning_menu_f1,2tuning_menu_f2,2tuning_menu_f3,2tuning_menu_f4,2tuning_menu_f5,2tuning_menu_f6,lambda: 2update_pid("Max speed",""),lambda: 2update_pid("base speed",""),0,0,f_exit,0,lambda: 2update_pid("",""),lambda: 2update_pid("ratio","throttle"),lambda: 2update_pid("ratio","enable_z"),lambda: 2update_pid("ratio","gyro"),lambda: 2update_pid("ratio","accel"),lambda: 2update_pid("g","xos"),lambda: 2update_pid("g","yos"),lambda: 2update_pid("g","zos"),lambda: 2update_pid("a","xos"),lambda: 2update_pid("a","yos"),lambda: 2update_pid("a","zos")]
	
	#tuning_menu=menu.submenu("TUNING second axis",menu_list,menu_functions,["1","2","3","4","5","6","7","8","t","b","e","m","0","a","s","d","f","y","u","i","o","p","["])
	#tuning_menu.IDLE_FUNCTION=idle_tuning
	#tuning_menu.USE_BANNER=0
	...
	#tuning test
	test_tuning_menu=menu.submenu("Test Tuning with current PID values",["Throttle: +10 press 5","          +1  press t","          -1  press g","          -10 press v","Use arrow keys to move and '/' to center","back: press SPACE"],[throttlepp,throttlep,throttlem,throttlemm,test_tuning_back,lambda: pitch_roll("f"),lambda: pitch_roll("b"),lambda: pitch_roll("r"),lambda: pitch_roll("l"),lambda: pitch_roll("z")],["5","t","g","v"," ","a","b","c","d","/"])
	#test_tuning_menu.ONCE_FUNCTION=lambda: start_motors(.1,base_speed)
	#test_tuning_menu.IDLE_FUNCTION=run
	
	test_tuning_menu.ONCE_FUNCTION=f_tuning_run
	test_tuning_menu.IDLE_FUNCTION=0
...
"""
def 2tuning_menu_f1()  : 2update_pid("g","p")
def 2tuning_menu_f2()  : 2update_pid("g","i")
def 2tuning_menu_f3()  : 2update_pid("g","d")
def 2tuning_menu_f4()  : 2update_pid("a","p")
def 2tuning_menu_f5()  : 2update_pid("a","i")
def 2tuning_menu_f6()  : 2update_pid("a","d")

"""
def test_tuning_back():
	"""
	global THROTTLE
	global ax_os_throttle
	global ay_os_throttle
	
	start_motors(0,0)
	print "Throttle is : {}".format(THROTTLE)
	print "X offset (LR) is : {}\nY offset (UD) is : {}".format(ax_os_throttle,ay_os_throttle)
	ax_os_throttle=0
	ay_os_throttle=0
	THROTTLE=0
	"""
...
def f_tuning_run():
	...
	print "Throttle is : {}".format(THROTTLE)
	print "X offset (LR) is : {}\nY offset (UD) is : {}".format(ax_os_throttle,ay_os_throttle)
	print "Yaw is {}".format(yaw_throttle)
	print "Updated {} times in {} seconds, {} per sec".format(update_counter, 0,0)
	ax_os_throttle=0
	ay_os_throttle=0
	THROTTLE=0
	yaw_throttle=0
...
def idle_tuning():
	...
	data_ax,data_ay,data_az=data_ax+ax_os+ax_os_throttle,data_ay+ay_os+ay_os_throttle,data_az+az_os
	
	data_gx = lp_data_gx.filter( data_gx)
	data_gy = lp_data_gy.filter( data_gy)
	data_gz = lp_data_gz.filter( data_gz)
	data_ax = lp_data_ax.filter( data_ax)
	data_ay = lp_data_ay.filter( data_ay)
	data_az = lp_data_az.filter( data_az)
...
def tune_run():
	...
	data_gx,data_gy,data_gz,t=Gyro.get() #degrees/sec
	data_ax,data_ay,data_az=Accel.get() #in G 	
	data_gx,data_gy,data_gz=data_gx+gx_os,data_gy+gy_os,data_gz+gz_os+yaw_throttle
	data_ax,data_ay,data_az=data_ax+ax_os+ax_os_throttle,data_ay+ay_os+ay_os_throttle,data_az+az_os
	...
	new_base=base_speed+MOTOR_SPEED_RATIO_throttle*THROTTLE
	new_gx=MOTOR_SPEED_RATIO_gyro*gx_up
	new_gy=MOTOR_SPEED_RATIO_gyro*gy_up
	new_gz=MOTOR_SPEED_RATIO_gyro*MOTOR_SPEED_RATIO_enable_z*gz_up
	new_ax=MOTOR_SPEED_RATIO_accel*ax_up
	new_ay=MOTOR_SPEED_RATIO_accel* ay_up
	#new_az=0*MOTOR_SPEED_RATIO_accel*MOTOR_SPEED_RATIO_enable_z* az_up
	
	pwm_br =new_base+(-new_gx-new_gy-new_gz)  +(-new_ax +new_ay)
	pwm_bl =new_base+(-new_gx+new_gy+new_gz)  +(+new_ax +new_ay)
	pwm_fl =new_base+(+new_gx+new_gy-new_gz)  +(+new_ax -new_ay)+17
	pwm_fr =new_base+(+new_gx-new_gy+new_gz)  +(-new_ax -new_ay)+30
...
def run():
	...
	pwm_br =new_base+(-new_gx-new_gy-new_gz)  +(-new_ax +new_ay+new_az)
	pwm_bl =new_base+(-new_gx+new_gy+new_gz)  +(+new_ax +new_ay+new_az)
	pwm_fl =new_base+(+new_gx+new_gy-new_gz)  +(+new_ax -new_ay+new_az)
	pwm_fr =new_base+(+new_gx-new_gy+new_gz)  +(-new_ax -new_ay+new_az)
...
def update_pid(device,param):
	...
	gy.setKp(pid_P_gyro)
	gz.setKp(pid_P_gyro)
	ax.setKp(pid_P_accel)
	ay.setKp(pid_P_accel)
	az.setKp(pid_P_accel)
	gx.setKi(pid_I_gyro)
	gy.setKi(pid_I_gyro)
	gz.setKi(pid_I_gyro)
	ax.setKi(pid_I_accel)
	ay.setKi(pid_I_accel)
	az.setKi(pid_I_accel)
	gx.setKd(pid_D_gyro)
	gy.setKd(pid_D_gyro)
	gz.setKd(pid_D_gyro)
	ax.setKd(pid_D_accel)
	ay.setKd(pid_D_accel)
	az.setKd(pid_D_accel)
...
lp_data_gx = LPfilter.LPfilter(LP_numPoints, data_gx)
lp_data_gy = LPfilter.LPfilter(LP_numPoints, data_gy)
lp_data_gz = LPfilter.LPfilter(LP_numPoints, data_gz)
lp_data_ax = LPfilter.LPfilter(LP_numPoints, data_ax)
lp_data_ay = LPfilter.LPfilter(LP_numPoints, data_ay)
lp_data_az = LPfilter.LPfilter(LP_numPoints, data_az)
ax_os_throttle=0
ay_os_throttle=0
