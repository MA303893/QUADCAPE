...
class PWM:
	#speed members are only used for checking; otherwise are useless
	speed1 = 0
        speed2 = 0
        speed3 = 0
        speed4 = 0
	...
	path3 = ''
	path4 = ''
	#these are strings to ease some typecasting
	startDuty = "750000"
	maxDuty = "2200000"
	unit = 0
	...
	def __init__(self):
		#find the unit
		self.unit = (float(self.maxDuty) - float(self.startDuty))/1000.0
	...
	#uncommented code is used to check if correct speed is being recieved by pwm
	...
	def changeSpeed1(self, speed):
		if speed >= 0 and speed <= 1000:
			...
			os.system("echo " + self.maxDuty + " > /sys/devices/" + self.path1 + "/duty")
	...
	def changeSpeed2(self, speed):
		if speed >= 0 and speed <= 1000:
			...
			os.system("echo " + self.maxDuty + " > /sys/devices/" + self.path2 + "/duty")
	...
	def changeSpeed3(self, speed):
		if speed >= 0 and speed <= 1000:
			...
			os.system("echo " + self.maxDuty + " > /sys/devices/" + self.path3 + "/duty")
	...
	def changeSpeed4(self, speed):
		if speed >= 0 and speed <= 1000:
			...
			os.system("echo " + self.maxDuty + " > /sys/devices/" + self.path4 + "/duty")
